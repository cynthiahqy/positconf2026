...
format:
  letterbox-revealjs: 
    standalone: true
    theme: [default, _style.scss, brand]
---

##

::: {.notes}
So naturally I reached for ggplot2, when I was overcome with the urge to do some procrasti-plotting.

Two years ago, I was lucky enough to get to go on a multi-month research trip including speaking at posit::conf(2024). Travelling to new pplaces, meeting new people? That's fun. Planning a trip and booking flights and hotels. Less fun. 

So I thought, what I really need to do is make an interactive overview of my trip. I had some dates sets, but there were days in between that were not yet accounted for. What better excuse to pull out R, do some calculations and wrap it all up in a nice pretty plot?
:::
...
```{.r filename="calendR-interface" code-line-numbers="|18-19"}
calendR(
...
```{.r filename="ggweekly-interface" code-line-numbers="|8-10"}
ggweek_planner(
...
```{.r filename="very-long-ggplot-script.R" code-line-numbers="|5-41|43-71|72-104|106-124"}
...
# Down the function design rabbit hole...
...
![](assets/notability-v2/principle-summary-v2-with-text.png){fig-alt="Modular internals, an exposed helper signature, and a documentation page shown together" fig-align="center"}
...
::: {.columns}
::: {.column .smallest}
```{.r code-line-numbers="43-71" filename="data-prep.R"}
...
```{.r code-line-numbers="|2-5|7-17|19-31" filename="internals"}
...
```{.r code-line-numbers="1-9|10-20" filename="function-interface"}
...
:::
::: {.column .small .fragment}
```{.r code-line-numbers="10-21" filename="function-interface"}
gg_facet_wrap_months(
  ## --- data ----
  .events_long,
  date_col,
...
::: {.notes}
Same helper, grouped by what the reader of the chart sees rather than by
ggplot2 component type. Note `.day_labels` merges both geom_text() calls —
the day number and the flag — because they are one visual idea.
:::
...
dates_df <- make_empty_month_days(
  c("2024-01-05", "2024-06-30"), dates_to = "date")
...
gg_facet_wrap_months(dates_df, date,
    .geom = list(
      geom_tile(color = "grey70", fill = "transparent"),
      geom_text(nudge_y = 0.25,
                color = "#6a329f")),
...
# Reusing happily ever after?

## From custom code...


![](assets/notability-v2/code-to-helper-01.png){.nostretch fig-align="center"}


## to transparent helpers...

![](assets/notability-v2/code-to-helper-03.png){.nostretch fig-align="center"}
...
Time is complicated, so is torturing ggplot2 to care about time.

Stay away if you value your sanity.
:::

::: {.fragment}
![](assets/temp/fig-ggtime-teaser.png){fig-align="center"}
:::
...
![](assets/temp/screenshot-skill.png)
...
::: {.sketch-container}
[🔗 Slides, links, skill & more: [github.com/cynthiahqy/positconf2026](https://github.com/cynthiahqy/positconf2026)]{.small}
:::

::: {.sketch-container .fragment}
📣 Tell me about your ggplot2 reuse experiences! 

![](assets/qrcode-survey.svg){.qr-code fig-alt="QR code linking to https://www.cynthiahqy.com/survey" fig-align="center"}
:::
